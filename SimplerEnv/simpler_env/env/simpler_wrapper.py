...
class NoraWrapper(SimlerWrapper):
    def __init__(self, args, unnorm_state, policy):
        super().__init__(args, unnorm_state)
        self.policy = policy

    def _process_action(self, raw_actions: torch.Tensor) -> torch.Tensor:
        output_action = self.policy.fast_tokenizer.decode([raw_actions[0] - self.policy.vla._ACTION_TOKEN_MIN])
        action_norm_stats = self.unnorm_state
        ...
        unnorm_actions = unnorm_actions[0]
        raw_action = {
            "world_vector": unnorm_actions[:, :3],
            "rotation_delta": unnorm_actions[:, 3:6],
            "open_gripper": unnorm_actions[:, 6:7],  # range [0, 1]; 1 = open; 0 = close
        }
        action = {}
        action["world_vector"] = raw_action["world_vector"]  # [B, 3]
        action["gripper"] = 2.0 * (raw_action["open_gripper"] > 0.5) - 1.0  # [B, 1]
        ...
        action = torch.cat([action["world_vector"], action["rot_axangle"], action["gripper"]], dim=1)
        #print("raw_actions:", raw_actions[0] - self.policy.vla._ACTION_TOKEN_MIN)
        #print("action:", action)
        # to tpdv
        action = action.to(raw_actions.device)
