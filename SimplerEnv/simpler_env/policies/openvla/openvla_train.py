...
class OpenVLAPPO:
    def __init__(self, all_args, policy: OpenVLAPolicy):
        ...
        self.ppo_value_coef = self.args.alg_value_coef
    ...
    def train_ppo_step(self, idx, total, batch):
        ...
        # Total loss
        loss = policy_loss #+ self.ppo_value_coef * value_loss - self.ppo_entropy_coef * entropy_loss
    ...
    def train_ppo_step_accum_steps(self, idx, total, batch):
        #get the ceil for the accum steps
        accum_steps = math.ceil(self.args.tt_steps / 8)
        obs_image, instruct, actions, value_preds, returns, masks, old_logprob, advantages = batch
        ...
        ratio_accum = 0
        ratio_median_accum = 0              
        ratio_2_accum = 0
        value_clip_ratio_accum = 0
        value_old_mean_accum = 0
        values_mean_accum = 0
        returns_mean_accum = 0
        returns_norm_mean_accum = 0
        logprob_mean_accum = 0
        logprob_old_mean_accum = 0

        for i in range(accum_steps):
            # Policy loss
            obs_slice = dict(image=obs["image"][i*8:(i+1)*8], task_description=obs["task_description"][i*8:(i+1)*8])
            logprob, entropy, values = self.policy.evaluate_actions(obs_slice, actions[i*8:(i+1)*8])

            ratio = torch.exp(logprob - old_logprob[i*8:(i+1)*8])
            surr1 = ratio * advantages[i*8:(i+1)*8]
            surr2 = torch.clamp(ratio, 1 - self.ppo_clip, 1 + self.ppo_clip) * advantages[i*8:(i+1)*8]
            policy_loss = -torch.min(surr1, surr2).sum(dim=-1, keepdim=True).mean()

            # Value loss
            value_pred_clipped = value_preds[i*8:(i+1)*8] + (values - value_preds[i*8:(i+1)*8]).clamp(-self.ppo_clip, self.ppo_clip)
            error_clipped = returns_norm[i*8:(i+1)*8] - value_pred_clipped
            error_original = returns_norm[i*8:(i+1)*8] - values
            value_loss_clipped = huber_loss(error_clipped, self.ppo_huber_delta)
            value_loss_original = huber_loss(error_original, self.ppo_huber_delta)
            value_loss = torch.max(value_loss_original, value_loss_clipped)

            value_clip_indicator = (value_pred_clipped - value_preds[i*8:(i+1)*8]).abs() > self.ppo_clip
            value_clip_ratio = value_clip_indicator.to(**self.tpdv).mean()
            ...
            policy_loss_accu += policy_loss.item()
            value_loss_accum += value_loss.item()
            entropy_loss_accum += entropy_loss.item()
            ratio_accum += ratio.mean().item()
            ratio_median_accum += ratio.median().item()
            ratio_2_accum += (logprob - old_logprob[i*8:(i+1)*8]).mean().exp().item()
            value_clip_ratio_accum += value_clip_ratio.item()
            value_old_mean_accum += value_preds[i*8:(i+1)*8].mean().item()
            values_mean_accum += values.mean().item()
            returns_mean_accum += returns[i*8:(i+1)*8].mean().item()
            returns_norm_mean_accum += returns_norm[i*8:(i+1)*8].mean().item()
            logprob_mean_accum += logprob.mean().item()
            logprob_old_mean_accum += old_logprob[i*8:(i+1)*8].mean().item()
        ...
        info = dict(
            loss=loss_accum,
            policy_loss=policy_loss_accu/accum_steps,
            value_loss=value_loss_accum/accum_steps,
            entropy_loss=entropy_loss_accum/accum_steps,
            ratio=ratio_accum/accum_steps,
            ratio_median=ratio_median_accum/accum_steps,
            ratio_2=ratio_2_accum/accum_steps,

            value_clip_ratio=value_clip_ratio_accum/accum_steps,
            value_old_mean=value_old_mean_accum/accum_steps,
            values_mean=values_mean_accum/accum_steps,
            returns_mean=returns_mean_accum/accum_steps,
            returns_norm_mean=returns_norm_mean_accum/accum_steps,
            logprob_mean=logprob_mean_accum/accum_steps,
            logprob_old_mean=logprob_old_mean_accum/accum_steps,
        )
        if grad_norm is not None:
            info["grad_norm"] = grad_norm.item()
    ...
    def train_ppo_vlac(self, buffer):
        train_info = defaultdict(lambda: [])
        ...
        batch = self.get_batch_from_buffer(buffer)

        idx = 0
        if self.args.tt_steps >8:
            info = self.train_ppo_step_accum_steps(idx, minibatch_count, batch)
        else:
            info = self.train_ppo_step(idx, minibatch_count, batch)
        for key, value in info.items():
            train_info[key].append(value)
    ...
    def get_batch_from_buffer(self, buffer):
        ...
        obs_batch = obs[buffer.step-buffer.tt_steps:buffer.step]
        actions_batch = actions[buffer.step-buffer.tt_steps:buffer.step]
        value_preds_batch = value_preds[buffer.step-buffer.tt_steps:buffer.step]
        return_batch = returns[buffer.step-buffer.tt_steps:buffer.step]
        masks_batch = masks[buffer.step-buffer.tt_steps:buffer.step]
        old_action_logits_batch = action_logits[buffer.step-buffer.tt_steps:buffer.step]
        adv_targ = advantages[buffer.step-buffer.tt_steps:buffer.step]
        instruct_batch = [buffer.instruction[0]] * obs_batch.shape[0]

        return (obs_batch, instruct_batch, actions_batch, value_preds_batch, return_batch, masks_batch,
                old_action_logits_batch, adv_targ)
    

IGNORE_INDEX = -100
class OpenVLAPerplexity:
    def __init__(self, all_args, policy: OpenVLAPolicy):
        self.args = all_args
        self.policy = policy
        self.tpdv = self.policy.tpdv
    ...
    def train_perplexity_step(self, idx, total, batch):
        obs_image, instruct, actions, value_preds, returns, masks, old_logprob, advantages = batch
        ...
        # here to calculate the perplexity loss
        # no need for action and set actiona to None
        features = self.policy._preprocess_obs(obs)
        features["labels"]= features["input_ids"].clone()

        label_len = features["labels"].shape[-1]
        
        outputs = self.policy.vla.forward(
        ...
        # only calculate the text part.
        sentence_ce = self.cal_ce(outputs.logits[:, -label_len:], features["labels"])  # [bs,]
        mask = sentence_ce > self.threshold  # Keep samples above threshold
        # 3. Calculate KL divergence
        sentence_kl = self.cal_kl(outputs.logits[:, -label_len:], features["labels"])  # [bs,]
        coeff = self.lamb * torch.exp(sentence_ce.clone().detach() - self.threshold) # [bs,]
            
        # 4. Compute total loss
        sentence_kl = sentence_kl.mul(coeff).mul(mask)  # [bs,]

        if mask.sum() == 0:
            loss = sentence_kl.mean()
        else:
            loss = sentence_kl.sum() / mask.sum()
    ...
    def train_perplexity_step_accum_steps(self, idx, total, batch):
        #get the ceil for the accum steps
        accum_steps = math.ceil(self.args.tt_steps / 8)
        obs_image, instruct, actions, value_preds, returns, masks, old_logprob, advantages = batch
    ...
    def train_perplexity(self, buffer):
        train_info = defaultdict(lambda: [])
        ...
        idx = 0
        if self.args.tt_steps >8:
            info = self.train_perplexity_step_accum_steps(idx, minibatch_count, batch)
        else:
            info = self.train_perplexity_step(idx, minibatch_count, batch)
        for key, value in info.items():
            train_info[key].append(value)
