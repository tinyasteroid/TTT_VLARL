...
from simpler_env.utils.replay_buffer import SeparatedReplayBuffer_perplexity

signal.signal(signal.SIGINT, signal.SIG_DFL)  # allow ctrl+c
os.environ["TOKENIZERS_PARALLELISM"] = "false"
import imageio

from simpler_env.policies.openvla.openvla_train import OpenVLAPolicy, OpenVLAPerplexity
...
class Runner:
    def __init__(self, all_args: Args):
        ...
        self.device_id = 0    # make sure data and policy on the same device
        self.device_id_other = 0 #if torch.cuda.device_count() > 1 else 0
        ...
        if self.args.vla_type == "openvla":
            ...
            self.alg = OpenVLAPerplexity(all_args, self.policy)
        ...
        self.buffer = SeparatedReplayBuffer_perplexity(
                self.args,
                obs_dim=(480, 640, 3),
                act_dim=7,     
            )
    ...
    def train(self):
        ...
        train_info = self.alg.train_perplexity(self.buffer)
