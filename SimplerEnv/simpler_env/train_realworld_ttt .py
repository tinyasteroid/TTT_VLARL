...
class Runner:
    ...
    def run(self, obs_img, instruction="put the bread on the plate"):
        ...
        if not torch.is_tensor(obs_img):
            obs_img = np.array(obs_img)    #[0-255].  h * w * 3
            obs_img = torch.tensor(obs_img).unsqueeze(0).to(self.device)
...
def main():
    ...
    #runner.run2(obj_set=args.obj_set)

    obs_img, instruction, info = runner.env.reset(obj_set=args.obj_set, same_init=args.use_same_init, seed=args.seed * 1000 + 0)
    for i in tqdm(range(args.episode_len), desc="rollout"):
        action = runner.run(obs_img=obs_img, instruction=instruction[0])
        #obs_img, reward, done, env_info = runner.env.step(action)
